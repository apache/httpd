...
curl -s $(ap1/print_address.sh)/folder1?F=0
curl -s $(ap1/print_address.sh)/folder1?F=0 | grep -C10 file1.dat | grep file2.dat
curl -s $(ap1/print_address.sh)/folder1?F=0 | tidy -quiet -errors
