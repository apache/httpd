...
if test -f ${DESTDIR}/usr/local2/apache2/bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/usr/local2/apache2/bin/apr-1-config
   APU_CONFIG=${DESTDIR}/usr/local2/apache2/bin/apu-1-config
else
...
test -n "/usr/local2/apache2/bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/usr/local2/apache2/build"
