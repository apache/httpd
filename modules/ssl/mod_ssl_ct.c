...
static int daemon_thread_start(apr_pool_t *pconf, server_rec *s_main)
{
    ...
    apr_pool_tag(pdaemon, "sct_daemon");
    rv = ap_thread_create(&daemon_thread, "mod_ssl_ct daemon", NULL,
                          sct_daemon_thread, s_main, pconf);
    if (rv != APR_SUCCESS) {
...
static void ssl_ct_child_init(apr_pool_t *p, server_rec *s)
{
    ...
    rv = ap_thread_create(&service_thread, "mod_ssl_ct service", NULL,
                          run_service_thread, s, p);
    if (rv != APR_SUCCESS) {
        ap_log_error(APLOG_MARK, APLOG_CRIT, rv, s,
