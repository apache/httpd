...
static ra_error r_assign_protocol(request_rec *r,
                                  const char *protocol,
                                  int strict)
{
    int proto_num;
    ra_error error = ra_none;
    ...
    if (protocol[0] == 'H' && protocol[1] == 'T'
        ...
        r->assbackwards = 0;
        proto_num = HTTP_VERSION(protocol[5] - '0', protocol[7] - '0');
        if (strict && error == ra_none)
            error = ra_badprotocol;
        else
                                  ...
                                  const char *protocol)
{
    ...
    ra_error error = r_assign_protocol(r, protocol, strict);
    ...
    if (r->proto_num == HTTP_VERSION(0, 9) && error == ra_none) {
        if (conf->http09_enable == AP_HTTP09_DISABLE)
            error = ra_reject09;
        else if (strict && (r->method_number != M_GET || r->header_only))
            error = ra_badmethod09;
    }
...
failed:
    if (error != ra_none && r->proto_num == HTTP_VERSION(0, 9)) {
...
request_rec *ap_read_request(conn_rec *conn)
{
    ...
    if (breq->headers) {
        r->headers_in = breq->headers;
    }
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r,
                  "checking request: %s %s %s",
                  breq->method, breq->uri, breq->protocol);

    if (!ap_assign_request(r, breq->method, breq->uri, breq->protocol)) {
        apr_brigade_cleanup(tmp_bb);
        ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r,
                      "error assigning request: %s %s", breq->method, breq->uri);
        switch (r->status) {
    ...
    apr_brigade_cleanup(tmp_bb);
    ap_log_rerror(APLOG_MARK, APLOG_TRACE2, 0, r,
                  "request assigned: %s %s", breq->method, breq->uri);
