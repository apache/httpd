                               ...
                               const char *at, int line)
{
    ap_log_cerror(APLOG_MARK, APLOG_INFO, status, cs->c, APLOGNO(10379)
                  "killing connection %" CS_FMT " at %s:%i",
                  CS_ARG(cs), at, line);
...
read_request:
    ...
    if (cs->pub.state == CONN_STATE_WRITE_COMPLETION) {
        ...
        if (pending == OK || (pending == DECLINED &&
            ...
            if (pollset_add(cs, cs->sc->wc_q)) {
                return;
            }

            /* fall through */
            apr_table_setn(cs->c->notes, "short-lingering-close", "1");
            pending = DONE; /* CONN_STATE_LINGER below */
        }
        if (pending != DECLINED
                || c->aborted
    ...
    if (cs->pub.state == CONN_STATE_CHECK_REQUEST_LINE_READABLE) {
        ...
        update_reqevents_from_sense(cs, CONN_SENSE_WANT_READ);
        if (pollset_add(cs, cs->ka_sc->ka_q)) {
            return;
        }
...
static void process_lingering_close(event_conn_state_t *cs)
{
    ...
    ap_log_cerror(APLOG_MARK, APLOG_TRACE6, 0, cs->c,
                  "lingering close for connection %" CS_FMT, CS_ARG(cs));
...
#define NON_WAKEABLE_POLL_TIMEOUT apr_time_from_msec(100)
         ...
         * Even if time2 drifted a bit since it was fetched and the real
